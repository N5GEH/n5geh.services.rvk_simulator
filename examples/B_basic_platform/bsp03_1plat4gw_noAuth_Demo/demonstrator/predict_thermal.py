...
#=======================================================================
    ...
    # ==================================================================
    def update_heat_consumption_from_crate(self, actual_time, time_step_in_s, arch_option_1, device_id, fnr):
        """ 
        ...
        wheres = "WHERE turnover_time_of_one_seg_in_h>={} AND turnover_time_of_one_seg_in_h<={} AND entity_id='{}'".format(start_time, end_time, device_id)
        ...
        # getting data from crate using orion context broker
            ...
            try:
                result = ant['rows']
                for line in result:
                    mytstamp = self.time_and_one_record_from_db(line, p_in_MPa, calc_opt)
            except:
    ...
    def get_last_storage_temp_profile_from_crate(actual_time, horizont_in_s, device_id):
        # get data for some short span of time in the past in case there were problems with the last record
        start_time = actual_time - timedelta(seconds=horizont_in_s)  
        end_time = actual_time + timedelta(seconds=(0.5 * 3600.0 * self.dt))  # until the current time - add offset for newest data coming in
        ...
        start_time = stt  # utc
        end_time = ett
        selects = "SELECT turnover_time_of_one_seg_in_h,T01_Sp01,T02_Sp02,T03_Sp03,T04_Sp04,T05_Sp05,T06_Sp06,T07_Sp07,T08_Sp08,T09_Sp09,T10_Sp10,T11_Sp11,T12_Sp12,T13_Sp13,T14_Sp14,T15_Sp15,T16_Sp16,T17_Sp17,T18_Sp18,T19_Sp19,T20_Sp20 FROM mtopeniot.etrvk"
        wheres = "WHERE turnover_time_of_one_seg_in_h>={} AND turnover_time_of_one_seg_in_h<={} AND entity_id='{}'".format(start_time, end_time, device_id)
        orders = "ORDER BY turnover_time_of_one_seg_in_h"
        sqlquery = "{} {} {}".format(selects, wheres, orders)
        payload = {'stmt': sqlquery}
        ...
        ant = r.json()
        try:
            wyn0 = ant['rows']  # all results
            n = len(wyn0)       # length of the list of the results - number of lines with results
            wyn1 = wyn0[n-1]    # last line - last result of the monitoring data saved in the db
            return (wyn1[1], wyn1[2], wyn1[3], wyn1[4], wyn1[5], wyn1[6], wyn1[7], wyn1[8], wyn1[9], wyn1[10], wyn1[11], wyn1[12], wyn1[13], wyn1[14], wyn1[15], wyn1[16], wyn1[17], wyn1[18], wyn1[19], wyn1[20])
        except:
            return -1
    # end get_last_storage_temp_profile_from_crate

    # ==================================================================

    def number_crate_records_in_time_span(self, actual_time, prediction_time_step_in_s, delta_in_seconds, device_id):
