...
class HeatStorageTank():
    ...
    def __init__(self, effective_height, inner_radius, effective_volume, effective_pipe_surface, effective_pipe_volume,
                 initial_temperature, alpha_loss, actual_time, el_heat_power, el_heat_status, timestepmanager, solverschema, 
                 elemtype, dbg_level, dbg_path):
        ...
        # 0 - no output; 1 - save temperature profiles at the end of each time step
        self.dbg = dbg_level
        self.dbg_path = dbg_path
        if(self.dbg == 1):
            G = open("{}/st_tp_avg.dat".format(self.dbg_path),"w")
            G.write(' temperature profiles in the storage tank \n')
            G.write(' dhw = domestic hot water, hw = heating water; all values in grad Celcius \n')
            G.write(' actual_time  time_in_h ')
            G.write(' dhw01 dhw02 dhw03 dhw04 dhw05 dhw06 dhw07 dhw08 dhw09 dhw10 dhw11 dhw12 dhw13 dhw14 dhw15 dhw16 dhw17 dhw18 dhw19 dhw20 ')
            G.write(' hw01 hw02 hw03 hw04 hw05 hw06 hw07 hw08 hw09 hw10 hw11 hw12 hw13 hw14 hw15 hw16 hw17 hw18 hw19 hw20 \n')
            G.flush()
            G.close()
        if(self.dbg == 2):
            G = open("{}/st_tp_full.dat".format(self.dbg_path),"w")
            G.write(' temperature profiles in the storage tank \n')
            G.write(' dhw = domestic hot water, hw = heating water; all values in grad Celcius \n')
            G.write(' actual time  time_in_h ')
            G.write(' dhw0 dhw01 dhw02 dhw03 dhw04 dhw05 dhw06 dhw07 dhw08 dhw09 dhw10 dhw11 dhw12 dhw13 dhw14 dhw15 dhw16 dhw17 dhw18 dhw19 dhw20 ')
            G.write(' dhw21 dhw22 dhw23 dhw24 dhw25 dhw26 dhw27 dhw28 dhw29 dhw30 dhw31 dhw32 dhw33 dhw34 dhw35 dhw36 dhw37 dhw38 dhw39 dhw40 ')
            G.write(' hw0 hw01 hw02 hw03 hw04 hw05 hw06 hw07 hw08 hw09 hw10 hw11 hw12 hw13 hw14 hw15 hw16 hw17 hw18 hw19 hw20 ')
            G.write(' hw21 hw22 hw23 hw24 hw25 hw26 hw27 hw28 hw29 hw30 hw31 hw32 hw33 hw34 hw35 hw36 hw37 hw38 hw39 hw40 \n')
            G.flush()
            G.close()
    ...
    def calculate_storage_tank(self, Timestep, hk_inp_temp, hk_inp_volfl_m3s, hk_out_temp, chp_inp_temp, chp_out_temp,
        ...
        """ obsolete procedure - temperatures of the water of heating water and domestic hot water in the tank """
...
#                    self.tout[ii] = self.temp[ii]
    ...
    def calculate_storage_tank_obj(self, tsm, hk_inp_temp, hk_inp_volfl_m3s, hk_out_temp, chp_inp_temp, chp_out_temp,
        ...
        # it is THE procedure for calculation of the temperatures in the storage tank
        # inp enters unit & leaves storage tank, out leaves unit & enters storage tank
        # hk - heating circuit, chp - chp unit, gb - gas boiler
        # temp - temperature; volfl - volume flow
...
#       ----------------------------------------------------------------
        if(self.dbg == 1):
            G = open("{}/st_tp_avg.dat".format(self.dbg_path),"a")
            G.write(' {}  {} '.format(current_time, utils.convert_time_to_hours(current_time)))
            for elem in self.t_dhw:
                G.write(' {} '.format(elem.get_average_temp()))
            for elem in self.t_hw:
                G.write(' {} '.format(elem.get_average_temp()))
            G.write(' \n')
            G.flush()
            G.close()
        if(self.dbg == 2):
            G = open("{}/st_tp_full.dat".format(self.dbg_path),"a")
            G.write(' {}  {} '.format(current_time, utils.convert_time_to_hours(current_time)))
            G.write(' {} '.format(self.t_dhw[0].get_input_temp()))
            for elem in self.t_dhw:
                G.write(' {} '.format(elem.get_average_temp()))
                G.write(' {} '.format(elem.get_output_temp()))
            G.write(' {} '.format(self.t_hw[0].get_input_temp()))
            for elem in self.t_hw:
                G.write(' {} '.format(elem.get_average_temp()))
                G.write(' {} '.format(elem.get_output_temp()))
            G.write(' \n')
...
#       ----------------------------------------------------------------
    # end calculate_storage_tank_obj
    ...
    def solve_hydraulic(self, t_new_dhw, t_new_hw, tinp_hw, tinp_dhw, in_idx_hw, in_idx_dhw, out_idx_hw, out_idx_dhw, el_heat_status, t_ambient):
        ...
        if(self.solverschema == 'implizit'):
            ...
            t_new_hw = self.electric_rod_heating(el_heat_status, t_new_hw)
...
#
    ...
    def electric_rod_heating(self, el_heat_status, t_hw):
        """ explicit scheme for heating up of the heating water only. """
        #self.tsm.get_timestep() # timestep in seconds
        #self.el_heat_status * self.el_heat_power # heating power in kW
        Q_kJ = self.el_heat_status * self.el_heat_power * self.tsm.get_timestep()  # in kJ = 1 * kW * s
        Q_kJ_i = Q_kJ
        #
        calc_opt_rho = 1   # Glueck
        # heating takes place in node 5 of the storage tank
        idx = 4           # 5-th layer, index == 4
        tlow = t_hw[idx]
        rho_low = utils.rho_fluid_water(tlow, utils.get_pressure_in_MPa(), calc_opt_rho)           # kg/m3
        cp_low = utils.cp_fluid_water(tlow, utils.get_pressure_in_MPa(), utils.get_calc_option())  # kJ/kg/K
        idx0 = idx + 1    #  == 5
        cges = rho_low * self.slice_volume * cp_low
        for ii in range(self.nr_slices - idx0):  # ii from 0 to 14
            jj = ii + idx0  # index of immediate upper layer  bsp: ii=0; jj=5; i.e. 6-th layer ;; ii=14; jj=19; i.e. 20-th layer
            if(t_hw[jj] > tlow):
                # upper layer is hotter than the lower one - as expected
                # heat up the lower layer to level of t_hw[jj]
                Q2j = cges * (t_hw[jj] - tlow)    # heat in kW needed for lower layer to reach the temperature of the upper layer
                if(Q_kJ_i > Q2j):
                    # there is more disposable heat than needed to heat up the lower layer
                    Q_kJ_i = Q_kJ_i - Q2j
                    tlow = t_hw[jj]
                else:
                    # lower layer can be only partially heated up by the disposable heat - there is no point in the further calculation
                    tlow = tlow + Q_kJ_i / cges
                    # set temperatures of all lower layers equal to the temperature that can be reached
                    # the current layer with index jj should not change the teperature
                    for kk in range(jj - idx0):
                        t_hw[kk + idx0] = tlow
                    Q_kJ_i = 0.0
                    break
            ...
            # if(t_hw[jj] > tlow):
                ...
                # upper layer is colder than the lower one - heat it up (t_hw[jj]) to the level of tlow
                rho_j = utils.rho_fluid_water(t_hw[jj], utils.get_pressure_in_MPa(), calc_opt_rho)           # kg/m3
                cp_j = utils.cp_fluid_water(t_hw[jj], utils.get_pressure_in_MPa(), utils.get_calc_option())  # kJ/kg/K
                Q2j = rho_j * self.slice_volume * cp_j * (tlow - t_hw[jj])  # kJ/K = kg/m3 * m3 * kJ/kg/K
                if(Q_kJ_i > Q2j):
                    # there is enough heat to reach tlow temperature
                    Q_kJ_i = Q_kJ_i - Q2j   # some of the disposable heat will be used up
                    t_hw[jj] = tlow         # upper layer will 
                # end if(Q_kJ_i > Q2j):
                else:
                    # upper layer can be only partially heated up - there is no point in further calculation
                    t_hw[jj] = t_hw[jj] + Q_kJ_i / (rho_j * self.slice_volume * cp_j)
                    Q_kJ_i = 0.0
                    break
                # end if(Q_kJ_i > Q2j): else:

            # end if(t_hw[jj] > tlow): else:

            # move the level up
            #tlow = t_hw[jj]
            rho_low = utils.rho_fluid_water(tlow, utils.get_pressure_in_MPa(), calc_opt_rho)           # kg/m3
            cp_low = utils.cp_fluid_water(tlow, utils.get_pressure_in_MPa(), utils.get_calc_option())  # kJ/kg/K
            # capacity of all layers that lie below th jj-th layer
            cges = cges + rho_low * self.slice_volume * cp_low  # kJ/K = kg/m3 * m3 * kJ/kg/K - add capacity of one more layer at the elevated temperature

        # end for ii in range(self.nr_slices - idx0)

        if(Q_kJ_i > 0.0):
            # disposable heat suffices to heat up the whole storage tank above layer 5 - possible when the temperature profile is flat
            tlow = tlow + Q_kJ_i / cges
            for ii in range(self.nr_slices - idx0):  # ii from 0 to 14
                t_hw[ii + idx0] = tlow
            # end for ii in range(self.nr_slices - idx0):
